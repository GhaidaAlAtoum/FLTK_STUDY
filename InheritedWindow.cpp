...
int main(){
	SimpleWindow win(300,200,"Simple Window");
	return Fl::run();
}
 ...
 SimpleWindow::SimpleWindow(int w, int h, const char* title):Fl_Window(w,h,title){
	  ...
	  inp = new Fl_Input(50, 50, 140, 30, "In:");
      out = new Fl_Output(50, 100, 140, 30, "Out:");
	  end();
   resizable(this);
   show();
}
...
/*
 * callbacks in a class can only be static, In other words the THIS pointer does not exist
 * 
 */
